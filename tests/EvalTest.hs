...
module EvalTest where
  ...
  valToTestVal (Fun _) = error "unexpected function construction"

  -- | Test if two Unsafe Int val is equal
  eqInt :: (Unsafe Val, [String]) -> (Unsafe Val, [String]) -> Bool
  eqInt (Ok (I n), _) (Ok (I m), _) = m == n
  eqInt _ _ = False

  -- | Test if two Unsafe Boolean is equal
  eqBool :: (Unsafe Val, [String]) -> (Unsafe Val, [String]) -> Bool
  eqBool (Ok (B b1), _) (Ok (B b2), _) = b1 == b2
  eqBool _ _ = False

  -- | Apply a unsafe function
  appFunc :: (Unsafe Val, [String]) -> (Unsafe Val, [String]) -> (Unsafe Val, [String])
  appFunc (Error msg, l) _ = (Error msg, l)
  appFunc _ (Error msg, l) = (Error msg, l)
  appFunc (Ok (Fun f), l) (Ok val, l') = case f val of
                                              (Ok a, l'') -> (Ok a, l' ++ l ++ l'')
  appFunc (Ok val, l) _ = (Error "first input is not a function in appFunc", l)

  -- | test if an unsafe is error
  isError :: (Unsafe a, [String]) -> Bool
  isError (Error _, _) = True
  isError _ = False

  -- | return if the two unsafe has the same error
  sameError :: (Unsafe a, [String]) -> (Unsafe a, [String]) -> Bool
  sameError (Error msg1, _) (Error msg2, _) = msg1 == msg2
  sameError _ _ = False

  -- | run a program under empty exprssion
  runEmpty :: Ast -> (Unsafe Val, [String])
  runEmpty exp = runEnvUnsafe (eval exp) Map.empty
  runEmptyT :: Ast -> (Unsafe TestVal, [String])
  runEmptyT exp =
    case runEnvUnsafe (eval exp) Map.empty of
      (Ok res, l) -> (Ok (valToTestVal res), l)
      (Error msg, l') -> (Error msg, l')
  ...
  evalTest = testGroup "test for evaluation function" [
    ...
    testProperty "function in list example: [True, 1, \\x -> x]" $
      \inp ->
        ...
        in
          case res of
            (Ok (Ls [B True, I 1, Fun f]), _) -> (f (I inp)) `eqInt` (Ok (I (inp)), [])
            _ -> False,
